...
class NeuralNetwork:
    ...
    Attributes:
        ...
        input (tensorflow Tensor)                   Input layer of the network. Represented as a placeholder
                                                        to allow the computation to be executed in a tensorflow
                                                        session
        training_y (tensorflow Tensor)              Training output data. Represented as a placeholder to
    ...
    def __init__(self, network, cost_func, activation_func, train_func, write_summaries=True):
        ...
        self.biases = []
        self.weights = []
        self.input = tf.placeholder(tf.float32, shape=[None, network[0]], name='input')
        self.training_y = tf.placeholder(tf.float32, shape=[None, network[len(network) - 1]], name='training_output')
            ...
            with tf.name_scope(layer_name):
                with tf.device('/cpu:0'):
                    with tf.name_scope('weights'):
                        weight = tf.Variable(tf.random_normal([network[i - 1], network[i]], stddev=0.05, name=layer_name + '/weights'))
                ...
                with tf.device('/gpu:0'):
                    with tf.name_scope('activation'):
                        activation = activation_func(tf.add(
                            tf.matmul(self.layer_values[i - 1], self.weights[i - 1]), self.biases[i - 1], name=layer_name + '/preactivation'),
                            name='activation')
        ...
        with tf.device('/gpu:0'):
            with tf.name_scope('cross_entropy'):
                cost = cost_func(logits=self.get_output_layer(), labels=self.training_y)
    ...
    def get_feed_dict(self, input_training, output_data):
        ...
        """
        return {self.input: input_training, self.training_y: output_data}
    ...
    def get_train_data(self):
        ...
        return self.input, self.training_y
